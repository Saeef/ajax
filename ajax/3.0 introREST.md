...
> cacheable therefore less runs to server and light weight. It represents also an
...
> added `Security`. When interacting with APIs, for example, it `usually involves` some 
> sort of `oAuth key` and a `secret name` that you `trade-in` for a `token` from the
